"""
Store autorun config manager.
Gestiona el arranque automático persistente del servicio store.
"""
...
class StoreAutorunConfigManager:
	"""Gestiona la persistencia de autorun del servicio store."""
	...
	def load_config(self) -> dict:
		...
		try:
			...
			logger.error(f"Error cargando autorun store: {exc}")

		return self._default_config()

	def save_config(self, autorun: bool) -> None:
		"""Guarda el estado de autorun del servicio store."""
		payload = {
		...
		try:
			...
			logger.error(f"Error guardando autorun store: {exc}")
...
def create_store_autorun_manager() -> StoreAutorunConfigManager:
	"""Factory de conveniencia para manager de autorun store."""
	return StoreAutorunConfigManager()
